...
function create_illusion(keys, illusion_origin, illusion_incoming_damage, illusion_outgoing_damage, illusion_duration)  
    ...
    local caster_level = keys.target:GetLevel()
    ...
    for ability_slot = 0, 15 do
        local individual_ability = keys.target:GetAbilityByIndex(ability_slot)
    ...
    for item_slot = 0, 5 do
        local individual_item = keys.target:GetItemInSlot(item_slot)
...
function item_manta_datadriven_on_spell_start(keys)
    if not keys.target:IsRangedAttacker() then  --Manta Style's cooldown is less for melee heroes.
        keys.ability:EndCooldown()
        keys.ability:StartCooldown(keys.CooldownMelee)
    end
    
    local manta_particle = ParticleManager:CreateParticle("particles/econ/items/kunkka/kunkka_weapon_shadow/kunkka_weapon_spell_tidebringer_shadow.vpcf", PATTACH_ABSORIGIN_FOLLOW, keys.target)
    ...
    keys.target:EmitSound("DOTA_Item.Manta.Activate")
    ...
    ProjectileManager:ProjectileDodge(keys.target)  --Disjoints disjointable incoming projectiles.
    
    --The caster is briefly made invulnerable and disappears, while ground vision is supplied nearby.
    keys.ability:CreateVisibilityNode(keys.target:GetAbsOrigin(), keys.VisionRadius, keys.InvulnerabilityDuration)
    keys.target:AddNoDraw()
    keys.ability:ApplyDataDrivenModifier(keys.target, keys.target, "modifier_item_manta_datadriven_invulnerability", nil)
end
...
function modifier_item_manta_datadriven_invulnerability_on_destroy(keys)
    local caster_origin = keys.target:GetAbsOrigin()
    ...
    local illusion1 = nil
    local illusion2 = nil
    if keys.target:IsRangedAttacker() then  --We don't have to worry about illusions switching from melee to ranged or vice versa because they can't use abilities.
    ...
    local caster_forward_vector = keys.target:GetForwardVector()
    ...
    if hero_random_origin == 1 then
        keys.target:SetAbsOrigin(caster_origin)
    ...
    elseif hero_random_origin == 2 then
        keys.target:SetAbsOrigin(illusion1_origin)
        ...
        keys.target:SetAbsOrigin(illusion2_origin)
        if illusion1_random_origin == 1 then
            illusion1:SetAbsOrigin(caster_origin)
            illusion2:SetAbsOrigin(illusion1_origin)
    ...
    local caster_health = keys.target:GetHealth()
    local caster_mana = keys.target:GetMana()
