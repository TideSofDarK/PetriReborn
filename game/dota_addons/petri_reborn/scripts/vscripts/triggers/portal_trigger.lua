...
function CheckFarmPlaces(trigger, activator)
	...
	if activator:GetUnitName() == "npc_dota_hero_storm_spirit" and not string.match(triggerName, "portal_trigger_creep0") then 
		return true 
	elseif activator:GetUnitName() == "npc_dota_hero_storm_spirit" and string.match(triggerName, "portal_trigger_creep0") then
		return false 
	end
