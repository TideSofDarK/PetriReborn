...
//=============================================================================
// Обновление панели КВНщиков
//=============================================================================
function _ScoreboardUpdater_UpdatePlayerPanelKVN( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
{
	...
	if ( playerPanel === null )
	{
		...
		playerPanel.BLoadLayout( scoreboardConfig.playerXmlNameKVN, false, false );
	}
	...
	if ( playerInfo )
	{
		...
		$.Msg( "playerItems = ", playerInfo.lumber);
		...
		_ScoreboardUpdater_SetTextSafe( playerPanel, "Gold", playerInfo.player_gold );
		_ScoreboardUpdater_SetTextSafe( playerPanel, "Lumber", playerInfo.lumber );
		_ScoreboardUpdater_SetTextSafe( playerPanel, "Food", playerInfo.food );
/*/
		_ScoreboardUpdater_SetTextSafe( playerPanel, "RespawnTimer", ( playerInfo.player_respawn_seconds + 1 ) ); // value is rounded down so just add one for rounded-up
		_ScoreboardUpdater_SetTextSafe( playerPanel, "PlayerName", playerInfo.player_name );
		_ScoreboardUpdater_SetTextSafe( playerPanel, "Level", playerInfo.player_level );
...
function _ScoreboardUpdater_UpdateTeamPanelKVN( scoreboardConfig, containerPanel, teamDetails )
{
	if ( !containerPanel )
		return;
	...
	if ( teamPanel === null )
	{
		//$.Msg( "UpdateTeamPanel.Create: ", teamPanelName, " = ", scoreboardConfig.teamXmlNameKVN );
		teamPanel = $.CreatePanel( "Panel", containerPanel, teamPanelName );
		teamPanel.SetAttributeInt( "team_id", teamId );
		teamPanel.BLoadLayout( scoreboardConfig.teamXmlNameKVN, false, false );
	...
	if ( playersContainer )
	{
		...
		{
			_ScoreboardUpdater_UpdatePlayerPanelKVN( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
		}
	}
...
//=============================================================================
// Обновление панели петросянов
//=============================================================================
function _ScoreboardUpdater_UpdatePlayerPanelPetro( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
{
	var playerPanelName = "_dynamic_player_" + playerId;
	var playerPanel = playersContainer.FindChild( playerPanelName );
	if ( playerPanel === null )
	{
		playerPanel = $.CreatePanel( "Panel", playersContainer, playerPanelName );
		playerPanel.SetAttributeInt( "player_id", playerId );
		playerPanel.BLoadLayout( scoreboardConfig.playerXmlNamePetro, false, false );
	}
...
function _ScoreboardUpdater_UpdateTeamPanelPetro( scoreboardConfig, containerPanel, teamDetails )
{
	if ( !containerPanel )
		return;
	...
	if ( teamPanel === null )
	{
		$.Msg( "UpdateTeamPanel.Create: ", teamPanelName, " = ", scoreboardConfig.teamXmlNamePetro );
		teamPanel = $.CreatePanel( "Panel", containerPanel, teamPanelName );
		teamPanel.SetAttributeInt( "team_id", teamId );
		teamPanel.BLoadLayout( scoreboardConfig.teamXmlNamePetro, false, false );
	...
	if ( playersContainer )
	{
		for ( var playerId of teamPlayers )
		{
			_ScoreboardUpdater_UpdatePlayerPanelPetro( scoreboardConfig, playersContainer, playerId, localPlayerTeamId )
		}
	}
...
//	$.Msg( "_ScoreboardUpdater_UpdateAllTeamsAndPlayers: ", scoreboardConfig );
	...
	// Панели для каждой из команд
	var panelsByTeam = [];
	panelsByTeam[ teamsList[0].team_id ] = _ScoreboardUpdater_UpdateTeamPanelKVN( scoreboardConfig, $( "#TeamKVN" ), teamsList[0] );
	panelsByTeam[ teamsList[1].team_id ] = _ScoreboardUpdater_UpdateTeamPanelPetro( scoreboardConfig, $( "#TeamPetro" ), teamsList[1] );
